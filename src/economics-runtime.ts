...
const USAGE_FIELDS = [
  ...
  "childAgents",
  "maxAgentDepth",
] as const satisfies readonly (keyof RuhrohEconomicUsageV1)[];

const TRACE_KINDS = new Set<RuhrohEconomicTraceSpanV1["kind"]>([
  "agent_turn",
  "inference",
  "tool",
  "retrieval",
...
const TRACE_STATUSES = new Set<RuhrohEconomicTraceSpanV1["status"]>([
  "ok",
  "error",
  "timeout",
  "cancelled",
...
export const RUHROH_RUNTIME_RESOURCE_CAPABILITIES: Readonly<Partial<Record<RuhrohEconomicResource, RuhrohResourceCapabilityV1>>> = {
...
export function validateEconomicsObservation(value: unknown): string[] {
  const errors: string[] = [];
  if (!isRecord(value)) {
  ...
  for (const key of Object.keys(value)) {
    if (!allowedObservationFields.has(key)) errors.push(`observation contains unsupported field ${key}`);
  }
  if (value.version !== "ruhroh_economics_observation_v1") {
    errors.push("version must be ruhroh_economics_observation_v1");
  }
  for (const field of ["observationId", "seriesId"] as const) {
  ...
  if (!isRecord(value.coverage) || !new Set(["complete", "partial", "unknown"]).has(String(value.coverage.status))) {
    ...
    if (Object.keys(value.coverage).some((key) => key !== "status" && key !== "missingReasons")) errors.push("coverage contains unsupported fields");
  ...
  if (!isRecord(value.source) || !nonEmptyString(value.source.name) || !nonEmptyString(value.source.kind) || !nonEmptyString(value.source.quality)) {
    ...
    if (Object.keys(value.source).some((key) => !new Set(["kind", "name", "quality", "observedAt", "priceBasisId"]).has(key))) errors.push("source contains unsupported fields");
  ...
  if (value.usage !== undefined) {
    if (!isRecord(value.usage)) {
      ...
      for (const key of Object.keys(value.usage)) {
        if (!(USAGE_FIELDS as readonly string[]).includes(key)) errors.push(`usage contains unsupported field ${key}`);
      }
  ...
  if (value.cost !== undefined) {
    if (!isRecord(value.cost)) {
      ...
      if (Object.keys(value.cost).some((key) => key !== "amount" && key !== "currency" && key !== "kind")) errors.push("cost contains unsupported fields");
...
export function validateRunAgentResultV2(value: unknown): string[] {
  if (!isRecord(value)) {
  ...
  for (const field of ["runId", "threadId", "adapterVersion"] as const) {
  ...
  if (value.economicsObservations !== undefined) {
    if (!Array.isArray(value.economicsObservations)) {
      ...
      const valid: RuhrohEconomicsObservationV1[] = [];
      for (const [index, observation] of value.economicsObservations.entries()) {
        ...
        if (itemErrors.length === 0) valid.push(observation as RuhrohEconomicsObservationV1);
      }
  ...
  if (value.economicTraceSpans !== undefined) {
    if (!Array.isArray(value.economicTraceSpans)) {
      ...
      const valid: RuhrohEconomicTraceSpanV1[] = [];
      for (const [index, span] of value.economicTraceSpans.entries()) {
        ...
        if (itemErrors.length === 0) valid.push(span as RuhrohEconomicTraceSpanV1);
      }
...
export function normalizeEconomicsObservations(
  observations: readonly RuhrohEconomicsObservationV1[],
): RuhrohEconomicsNormalizationResult {
  const errors: string[] = [];
  ...
  for (const [index, observation] of observations.entries()) {
    ...
    if (observationId !== undefined) seenObservationIds.add(observationId);
  ...
  const usageTotals: RuhrohEconomicUsageV1 = {};
  for (const field of USAGE_FIELDS) {
  ...
  const coverage: Partial<Record<RuhrohEconomicResource, RuhrohEconomicMetricCoverageV1>> = {};
  for (const field of USAGE_FIELDS) {
...
export function legacyUsageToEconomicsEnvelope(value: unknown): RuhrohEconomicsEnvelopeV1 | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
  const usage: RuhrohEconomicUsageV1 = {};
  ...
  const cost = nonNegativeFiniteNumber(value.costUsd)
    ? { amount: Number(value.costUsd), currency: "USD", kind: "manual" as const }
    : undefined;
  if (Object.keys(usage).length === 0 && cost === undefined) {
    return undefined;
  }
  const observation: RuhrohEconomicsObservationV1 = {
    version: "ruhroh_economics_observation_v1",
...
export function validateEconomicTraceSpan(value: unknown): string[] {
  ...
  if (value.parentSpanId !== undefined && !TRACE_ID_PATTERN.test(String(value.parentSpanId))) errors.push("parentSpanId must be a safe opaque identifier");
  if (value.links !== undefined && (!Array.isArray(value.links) || value.links.some((link) => !TRACE_ID_PATTERN.test(String(link))) || new Set(value.links).size !== value.links.length)) errors.push("links must contain unique safe opaque identifiers");
  if (!TRACE_KINDS.has(value.kind as RuhrohEconomicTraceSpanV1["kind"])) {
    errors.push("kind is not a supported economic trace span kind");
  }
  if (!TRACE_STATUSES.has(value.status as RuhrohEconomicTraceSpanV1["status"])) {
  ...
  if (value.agent !== undefined) {
    if (!isRecord(value.agent)) {
      ...
      for (const key of Object.keys(value.agent)) {
        if (!allowed.has(key)) errors.push(`agent contains unsupported field ${key}`);
      }
      for (const key of ["agentIdHash", "parentAgentIdHash"] as const) {
        if (value.agent[key] !== undefined && !/^[a-f0-9]{64}$/u.test(String(value.agent[key]))) {
          errors.push(`agent.${key} must be a lowercase SHA-256`);
        }
      }
      if (value.agent.depth !== undefined && (!Number.isInteger(value.agent.depth) || Number(value.agent.depth) < 0)) errors.push("agent.depth must be a non-negative integer");
      if (value.agent.adapterId !== undefined && !nonEmptyString(value.agent.adapterId)) errors.push("agent.adapterId must be a non-empty string");
    }
  }
  if (value.inference !== undefined) {
    if (!isRecord(value.inference)) {
      ...
      for (const key of Object.keys(value.inference)) {
        if (!allowed.has(key)) errors.push(`inference contains unsupported field ${key}`);
      }
      for (const key of ["routeHash", "requestIdHash"] as const) {
      ...
      for (const key of ["provider", "model", "modelVersion"] as const) {
        if (value.inference[key] !== undefined && !nonEmptyString(value.inference[key])) errors.push(`inference.${key} must be a non-empty string`);
      }
    }
  }
  return unique(errors);
...
export function validateEconomicTrace(spans: readonly RuhrohEconomicTraceSpanV1[]): string[] {
  const errors: string[] = [];
  const byId = new Map<string, RuhrohEconomicTraceSpanV1>();
  ...
  for (const span of spans) {
    ...
    let cursor: RuhrohEconomicTraceSpanV1 | undefined = span;
...
export function finalizeEconomicTraceJsonl(input: string | Buffer): RuhrohFinalizedEconomicTraceJsonl {
  ...
  const spans: RuhrohEconomicTraceSpanV1[] = [];
  const errors: string[] = [];
  ...
  for (const [index, rawLine] of lines.entries()) {
    ...
    if (spanErrors.length === 0) spans.push(cloneJson(parsed) as RuhrohEconomicTraceSpanV1);
  }
...
export function validateAdapterManifest(value: unknown): string[] {
  const errors: string[] = [];
  if (!isRecord(value)) {
...
export function validateResourceBudgets(value: unknown): string[] {
  const errors: string[] = [];
  if (!isRecord(value)) {
    return ["resource budgets must be an object"];
  }
  for (const key of Object.keys(value)) {
    if (!new Set(["version", "scope", "onUnobservable", "limits"]).has(key)) errors.push(`resource budgets contain unsupported field ${key}`);
  }
  ...
  for (const [index, rawLimit] of value.limits.entries()) {
    ...
    for (const key of Object.keys(rawLimit)) {
      if (!new Set(["resource", "max", "currency", "requiredEnforcement"]).has(key)) errors.push(`limits[${index}] contains unsupported field ${key}`);
    }
...
export function validateResourceBudgetOutcome(value: unknown): string[] {
  if (!isRecord(value)) return ["resource budget outcome must be an object"];
  const errors: string[] = [];
  ...
  for (const [index, limit] of value.limits.entries()) {
    ...
    if (!nonNegativeFiniteNumber(limit.limit)) errors.push(`limits[${index}].limit must be non-negative`);
    if (limit.observed !== undefined && !nonNegativeFiniteNumber(limit.observed)) errors.push(`limits[${index}].observed must be non-negative`);
    if (limit.enforcement !== "preemptive" && limit.enforcement !== "boundary") errors.push(`limits[${index}].enforcement is invalid`);
    if (!new Set(["complete", "partial", "unknown", "unavailable"]).has(String(limit.coverage))) errors.push(`limits[${index}].coverage is invalid`);
    if (!new Set(["within", "exhausted", "overrun", "unobservable"]).has(String(limit.status))) errors.push(`limits[${index}].status is invalid`);
  }
...
export function validateProcessTermination(value: unknown): string[] {
  if (!isRecord(value)) return ["process termination must be an object"];
  const errors: string[] = [];
  ...
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) errors.push(`process termination contains unsupported field ${key}`);
  }
  if (value.version !== "ruhroh_process_termination_v1") errors.push("version must be ruhroh_process_termination_v1");
  if (value.scope !== "process_group" && value.scope !== "process") errors.push("scope must be process_group or process");
  if (value.reason !== "wall_time_limit" && value.reason !== "iteration_timeout") errors.push("reason must be wall_time_limit or iteration_timeout");
  for (const field of ["timeoutMs", "timeoutObservedAtMs", "terminationDurationMs", "terminatedAtMs", "limitMs", "overrunMs"] as const) {
    if (value[field] !== undefined && !nonNegativeFiniteNumber(value[field])) errors.push(`${field} must be a non-negative finite number`);
  }
  if (value.gracePeriodMs !== 5000) errors.push("gracePeriodMs must be 5000");
...
export function budgetCapabilityErrors(
  manifest: RuhrohAdapterManifestV1 | undefined,
  budgets: RuhrohResourceBudgetsV1,
): string[] {
...
export function evaluateResourceBudgets(
  budgets: RuhrohResourceBudgetsV1,
  observed: RuhrohObservedResources,
  capabilities: Partial<Record<RuhrohEconomicResource, RuhrohResourceCapabilityV1>> = {},
  completed = false,
): RuhrohResourceBudgetOutcomeV1 {
  const outcomes: RuhrohResourceBudgetLimitOutcomeV1[] = budgets.limits.map((limit) => {
    ...
    let status: RuhrohResourceBudgetLimitOutcomeV1["status"] = "within";
...
export function runAdapterConformance(input: {
  ...
  const checks: RuhrohAdapterConformanceV1["checks"] = [];
  const addCheck = (name: string, errors: string[]): void => {
    checks.push({
      name,
...
function normalizeMetric(
  observations: readonly RuhrohEconomicsObservationV1[],
  field: keyof RuhrohEconomicUsageV1,
  errors: string[],
): number | undefined {
  ...
  let total = 0;
  const cumulative = new Map<string, RuhrohEconomicsObservationV1[]>();
...
function normalizeMaximumMetric(
  observations: readonly RuhrohEconomicsObservationV1[],
  field: "maxAgentDepth",
  errors: string[],
): number | undefined {
  ...
  for (const [seriesId, series] of cumulative) {
    ...
    for (const observation of [...series].sort((left, right) => left.sequence - right.sequence)) {
      const value = Number(observation.usage?.[field] ?? 0);
      if (value < previous) errors.push(`cumulative series ${seriesId} decreased for ${field} at sequence ${observation.sequence}`);
      previous = value;
    }
  }
...
function validateSeriesContracts(observations: readonly RuhrohEconomicsObservationV1[], errors: string[]): void {
  const series = new Map<string, RuhrohEconomicsObservationV1[]>();
...
function observationCapabilityErrors(
  manifest: RuhrohAdapterManifestV1,
  observations: readonly RuhrohEconomicsObservationV1[],
): string[] {
  const observed = new Set<RuhrohEconomicResource>();
  for (const observation of observations) {
    for (const field of USAGE_FIELDS) {
    ...
    if (observation.cost !== undefined) observed.add("cost");
  }
  return [...observed]
...
function normalizeCosts(
  observations: readonly RuhrohEconomicsObservationV1[],
  errors: string[],
): RuhrohEconomicCostV1[] {
  const totals = new Map<string, { amount: number; kinds: Set<RuhrohEconomicCostV1["kind"]> }>();
  ...
  for (const observation of additive) {
    ...
    const state = totals.get(cost.currency) ?? { amount: 0, kinds: new Set<RuhrohEconomicCostV1["kind"]>() };
  ...
  for (const [seriesKey, series] of cumulative) {
    ...
    if (currency !== undefined) {
      const state = totals.get(currency) ?? { amount: 0, kinds: new Set<RuhrohEconomicCostV1["kind"]>() };
...
function strongestCostKind(kinds: ReadonlySet<RuhrohEconomicCostV1["kind"]>): RuhrohEconomicCostV1["kind"] {
  for (const kind of ["billed", "metered", "estimated", "manual"] as const) {
    if (kinds.has(kind)) {
      return kind;
    }
  }
...
function summarizeMetricCoverage(
  observations: readonly RuhrohEconomicsObservationV1[],
  invalid: boolean,
): RuhrohEconomicMetricCoverageV1 {
...
function tracePrivacyErrors(value: unknown, path = "span"): string[] {
...
function enforcementSatisfies(actual: RuhrohBudgetEnforcement, required: Exclude<RuhrohBudgetEnforcement, "unsupported">): boolean {
  return actual === "preemptive" || actual === required;
}

function isEconomicResource(value: unknown): value is RuhrohEconomicResource {
  return typeof value === "string" && (RUHROH_ECONOMIC_RESOURCE_NAMES as readonly string[]).includes(value);
}
