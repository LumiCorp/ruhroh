...
import {
  ...
  type RuhrohScenario,
  type RuhrohScenarioEvaluationCalibrationSummary,
  type RuhrohScenarioEvaluationLintDiagnostic,
  type RuhrohScenarioSource,
} from "./scenarios.js";
...
export function validateRuhrohScenarioSource(input: string | RuhrohScenarioSource): ValidateRuhrohScenarioSourceResult {
  ...
  const warningDetails: RuhrohScenarioEvaluationLintDiagnostic[] = [];
...
function scenarioGovernanceWarnings(scenario: RuhrohScenario): string[] {
  const warnings: string[] = [];
...
function readEvaluationPrivateAssets(value: unknown): unknown {
  return isRecord(value) ? value.privateAssets : undefined;
}

function validateDeclaredAssetPaths(value: unknown, source: RuhrohScenarioSource, field: string): string[] {
  if (value === undefined) {
    return [];
  }
  if (!Array.isArray(value)) {
...
function validatePrivateAssetExposure(publicValue: unknown, privateValue: unknown, source: RuhrohScenarioSource): string[] {
...
function copyPrivateEvaluationAssets(
  scenario: RuhrohScenario,
  scenarioDir: string,
  taskDir: string,
): string[] {
