...
import type {
  ...
  RuhrohBenchmarkSuiteV2,
  RuhrohEfficiencyObjective,
  RuhrohSuiteEfficiencyContract,
} from "./suites.js";
...
export function buildRuhrohOutcomeFrontier(input: BuildRuhrohOutcomeFrontierInput): RuhrohOutcomeFrontier {
  ...
  const targetMap = new Map<string, RuhrohRunSummary[]>();
  ...
  const comparableTargets = qualityEligibleTargets.filter((target) =>
    target.objectives.length === suite.methodology.efficiency.objectives.length
  ...
  const status: RuhrohFrontierStatus = reasonCodes.length === 0 ? "available" : "unavailable";
...
function buildFrontierTarget(
  benchmarkTargetId: string,
  summaries: RuhrohRunSummary[],
  suite: RuhrohBenchmarkSuiteV2,
): RuhrohOutcomeFrontierTarget {
  ...
  const scenarioResults = suite.scenarioIds.map((scenarioId) =>
    buildScenarioQuality(scenarioId, summaries.filter((summary) => summary.scenarioId === scenarioId), suite)
  );
  const floorStatus: RuhrohQualityFloorStatus = scenarioResults.some((result) => result.floorStatus === "indeterminate")
    ? "indeterminate"
  ...
  const objectives = suite.methodology.efficiency.objectives.map((objective) =>
    buildObjectiveEstimate(objective, benchmarkTargetId, summaries, suite.scenarioIds)
  );
  return {
    benchmarkTargetId,
...
function buildScenarioQuality(
  scenarioId: string,
  summaries: RuhrohRunSummary[],
  suite: RuhrohBenchmarkSuiteV2,
): RuhrohFrontierScenarioQuality {
  ...
  const floorStatus: RuhrohQualityFloorStatus = reasonCodes.length > 0
    ? "indeterminate"
...
function buildObjectiveEstimate(
  objective: RuhrohEfficiencyObjective,
  benchmarkTargetId: string,
  summaries: RuhrohRunSummary[],
  scenarioIds: readonly string[],
): RuhrohFrontierObjectiveEstimate {
  const acceptedOutcomes = summaries.filter((summary) => summary.acceptedOutcome).length;
  ...
  const status: RuhrohObjectiveStatus = coverage.status === "partial"
    ? "partial"
...
function bootstrapObjectiveValues(
  objective: RuhrohEfficiencyObjective,
  benchmarkTargetId: string,
  summaries: RuhrohRunSummary[],
  scenarioIds: readonly string[],
  ...
  for (let sampleIndex = 0; sampleIndex < BOOTSTRAP_RESAMPLES; sampleIndex += 1) {
    const sample: RuhrohRunSummary[] = [];
    for (const [stratumIndex, stratum] of strata.entries()) {
      for (let drawIndex = 0; drawIndex < stratum.length; drawIndex += 1) {
        const selectedIndex = Math.floor(
          seededUnitInterval(`${benchmarkTargetId}:${objective}:${stratumIndex}`, sampleIndex, drawIndex) * stratum.length,
        );
...
function applyDominance(
  targets: RuhrohOutcomeFrontierTarget[],
  comparableTargets: RuhrohOutcomeFrontierTarget[],
  objectives: readonly RuhrohEfficiencyObjective[],
): void {
  ...
  for (const target of targets) {
    ...
    const pointDominators = comparableTargets.filter((candidate) =>
      candidate.benchmarkTargetId !== target.benchmarkTargetId && pointDominates(candidate, target, objectives)
    );
    const robustDominators = comparableTargets.filter((candidate) =>
      candidate.benchmarkTargetId !== target.benchmarkTargetId && robustlyDominates(candidate, target, objectives)
    );
...
function pointDominates(
  candidate: RuhrohOutcomeFrontierTarget,
  target: RuhrohOutcomeFrontierTarget,
  objectives: readonly RuhrohEfficiencyObjective[],
): boolean {
  let strict = false;
...
function findObjective(target: RuhrohOutcomeFrontierTarget, objective: RuhrohEfficiencyObjective): RuhrohFrontierObjectiveEstimate | undefined {
  return target.objectives.find((estimate) => estimate.objective === objective);
}

function objectiveValues(objective: RuhrohEfficiencyObjective, summaries: readonly RuhrohRunSummary[]): number[] {
...
function objectiveValue(objective: RuhrohEfficiencyObjective, summary: RuhrohRunSummary): number {
  if (objective === "cost_per_accepted_outcome") {
...
function objectivePointEstimate(
  objective: RuhrohEfficiencyObjective,
  summaries: readonly RuhrohRunSummary[],
): number | undefined {
  ...
  if (objective === "p95_implementation_wall_time_ms") {
    return weightedPercentile(
      summaries.map((summary) => ({ value: summary.implementationWallTimeMs ?? 0, weight: summary.sample?.weight ?? 0 })),
      0.95,
    );
  }
  const weightedAcceptedOutcomes = summaries.reduce(
    (total, summary) => total + (summary.acceptedOutcome ? (summary.sample?.weight ?? 0) : 0),
    0,
  );
  ...
  return summaries.reduce(
    (total, summary) => total + objectiveValue(objective, summary) * (summary.sample?.weight ?? 0),
    0,
  ) / weightedAcceptedOutcomes;
}

function objectiveUnit(objective: RuhrohEfficiencyObjective): RuhrohFrontierObjectiveEstimate["unit"] {
  if (objective === "cost_per_accepted_outcome") {
...
function coverageForObjective(
  ...
  const statuses = objective === "p95_implementation_wall_time_ms"
    ? summaries.map((summary) => summary.implementationWallTimeMs === undefined ? "unavailable" as const : "complete" as const)
...
function coverageForPlanWeights(summaries: readonly RuhrohRunSummary[]): RuhrohMetricCoverage {
...
function weightedPercentile(values: readonly { value: number; weight: number }[], percentile: number): number {
...
function bootstrapConfidenceInterval(values: number[]): RuhrohConfidenceInterval {
...
function seededUnitInterval(seed: string, sampleIndex: number, drawIndex: number): number {
...
function emptyCoverage(targetCount: number): RuhrohOutcomeFrontier["coverage"] {
  return {
    targetCount,
...
export function validateRuhrohOutcomeFrontier(input: unknown): RuhrohOutcomeFrontierValidationResult {
  const errors: string[] = [];
  ...
  for (const [index, target] of targets.entries()) {
    ...
    if (selectedObjectives !== undefined) {
      const targetObjectives = (Array.isArray(target.objectives)
        ? target.objectives.filter((objective): objective is Record<string, unknown> => isRecord(objective))
        : [])
...
function validateFrontierTarget(target: Record<string, unknown>, errors: string[], pathLabel: string): void {
...
function validateCoverage(coverage: Record<string, unknown>, errors: string[], pathLabel: string): void {
...
function requireRecordArray(record: Record<string, unknown>, field: string, errors: string[], pathLabel: string): Record<string, unknown>[] {
  const value = record[field];
  if (!Array.isArray(value)) {
...
function requireStringArray(record: Record<string, unknown>, field: string, errors: string[], pathLabel: string): string[] {
  const value = record[field];
  if (!Array.isArray(value)) {
...
function requireNonEmptyString(record: Record<string, unknown>, field: string, errors: string[], pathLabel: string): string | undefined {
  const value = record[field];
...
function requireNonNegativeNumber(record: Record<string, unknown>, field: string, errors: string[], pathLabel: string): void {
  const value = record[field];
