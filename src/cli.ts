...
interface RuhrohClaimSourceVerificationCheck {
  name: string;
  status: "ok" | "warning" | "failed";
  details: string;
  path?: string | undefined;
...
interface RuhrohClaimSourceVerificationReport {
...
function verifyBenchmarkClaimSources(claim: Record<string, unknown>, claimPath: string): RuhrohClaimSourceVerificationReport {
...
function runPlanCoverageWarnings(
  ...
  const warnings = expectedSamples.flatMap((sample) =>
    presentSampleIds.has(sample.sampleId)
      ? []
...
function scaffoldRuhrohProject(rootDir: string): RuhrohInitFile[] {
...
function scaffoldRuhrohAdapter(input: { adapterRoot: string; id: string }): RuhrohInitFile[] {
...
function adapterRunScript(adapterId: string): string {
...
function adapterReadme(adapterId: string): string {
