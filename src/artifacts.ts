export type RuhrohEvidenceTruthPlane =
  | "technical"
  | "human_work"
  | "billing"
  | "decision"
...
export const RUHROH_EVIDENCE_ARTIFACT_ROLES = {
  ...
  "billing-source-manifest-v2": descriptor("ruhroh_billing_source_manifest_v2", "billing", "restricted", "billing-source-manifest-v2.json"),
  "billing-mapping-profile-v2": descriptor("ruhroh_billing_mapping_profile_v2", "billing", "restricted", "billing-mapping-profile-v2.json"),
  "normalized-billing-row-v2": descriptor("ruhroh_normalized_billing_row_v2", "billing", "restricted", "normalized-billing-row-v2.ndjson"),
  "cost-reconciliation-v2": descriptor("ruhroh_cost_reconciliation_v2", "billing", "allowed", "cost-reconciliation-v2.json"),
  "focus-spec-lock": descriptor("ruhroh_focus_spec_lock_v1", "billing", "allowed", "focus-spec-lock.json"),
  "focus-catalog": descriptor("ruhroh_focus_catalog_v1", "billing", "allowed", "focus-catalog.json"),
  "focus-mapping-pack": descriptor("ruhroh_focus_mapping_pack_v1", "billing", "allowed", "focus-mapping-pack.json"),
  "focus-conformance-profile": descriptor("ruhroh_focus_conformance_profile_v1", "billing", "allowed", "focus-conformance-profile.json"),
  "focus-conformance-report": descriptor("ruhroh_focus_conformance_report_v1", "billing", "allowed", "focus-conformance-report.json"),
  "focus-validator-output": descriptor("focus_validator_junit_raw", "billing", "restricted", "focus-validator.xml"),
  "focus-source-dataset": descriptor("focus_source_dataset_raw", "billing", "restricted", "focus-source-data"),
  "focus-dataset-bundle": descriptor("ruhroh_focus_dataset_bundle_v1", "billing", "restricted", "focus-dataset-bundle.json"),
  "focus-attribution-profile": descriptor("ruhroh_focus_attribution_profile_v1", "billing", "restricted", "focus-attribution-profile.json"),
  "focus-import-report": descriptor("ruhroh_focus_import_report_v1", "billing", "allowed", "focus-import-report.json"),
  "focus-update-review": descriptor("ruhroh_focus_update_review_v1", "billing", "allowed", "focus-update-review.json"),
...
} as const satisfies Record<string, RuhrohEvidenceArtifactRoleDescriptor>;
...
export function evidenceArtifactRoleDescriptor(
  role: RuhrohEvidenceArtifactRole,
): RuhrohEvidenceArtifactRoleDescriptor {
  return { ...RUHROH_EVIDENCE_ARTIFACT_ROLES[role] };
}

export function validateRuhrohEvidenceArtifactReference(
  value: unknown,
  options: { publicBundle?: boolean | undefined } = {},
): string[] {
  if (!isRecord(value)) return ["artifact reference must be an object"];
  const errors: string[] = [];
  ...
  const role = typeof value.role === "string" && value.role in RUHROH_EVIDENCE_ARTIFACT_ROLES
    ? value.role as RuhrohEvidenceArtifactRole
    : undefined;
  if (role === undefined) {
...
export function validateRuhrohPublicArtifactInventory(
  references: readonly RuhrohEvidenceArtifactReferenceV1[],
): string[] {
  const errors = references.flatMap((reference, index) =>
    validateRuhrohEvidenceArtifactReference(reference, { publicBundle: true })
      .map((error) => `artifacts[${index}]: ${error}`)
  );
  const roles = references.map((reference) => reference.role);
  if (new Set(roles).size !== roles.length) errors.push("public artifact roles must be unique");
  return unique(errors);
}
