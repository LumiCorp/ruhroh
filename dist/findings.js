...
export function assessRuhrohFinding(input) {
    ...
    const status = !detector.observable
        ? "not_observable"
        : confirmed
            ? "confirmed"
            : "candidate";
...
export function buildRuhrohFindings(inputs, createdAt = new Date().toISOString()) {
...
export function validateRuhrohFindings(value) {
    if (!isRecord(value)) {
    ...
    for (const [index, raw] of value.findings.entries()) {
        ...
        if (!RUHROH_FINDING_DETECTORS.includes(raw.detectorId)) {
        ...
        if (raw.status === "confirmed") {
            ...
            for (const field of ["signatureObserved", "controlledCountercase", "equalQuality", "relevantCoverageComplete"]) {
...
function assessDetector(detectorId, measurements) {
    switch (detectorId) {
        case "context_amplification": {
            ...
            return observed(rising && (measurements.repeatedInputTokens ?? 0) > 0, `turn input tokens ${rising ? "rose monotonically" : "did not rise monotonically"}; repeated input tokens=${measurements.repeatedInputTokens ?? 0}`, "Run a controlled compacted-context condition with identical target, task, tools, and outcome floor.");
        }
        case "retry_loop_amplification": {
            ...
            return observed(measurements.equivalentRetryCount > 0, `equivalent retries=${measurements.equivalentRetryCount} of total retries=${measurements.retryCount}`, "Compare the same workload with a predeclared equivalent-retry breaker while preserving the quality floor.");
        }
        case "unnecessary_reasoning": {
            ...
            return observed(measurements.lowerReasoningTokens < measurements.reasoningTokens, `reasoning tokens baseline=${measurements.reasoningTokens}, countercase=${measurements.lowerReasoningTokens}`, "Repeat both reasoning conditions across the same frozen sample plan and confirm equal quality with complete coverage.");
        }
        case "cache_misuse": {
            ...
            return observed(measurements.cacheEligibleInputTokens > 0 && measurements.cachedInputTokens === 0, `cache-eligible tokens=${measurements.cacheEligibleInputTokens}, cached tokens=${measurements.cachedInputTokens}`, "Run a provider-verified exact-prefix countercase and measure accepted-outcome tokens and cost.");
        }
        case "rework": {
            ...
            return observed(measurements.revertedOrReplacedChanges > 0, `reverted or replaced changes=${measurements.revertedOrReplacedChanges}`, "Compare against a controlled validation-before-edit condition with the same quality and task set.");
        }
        case "unpinned_model_alias": {
            ...
            return observed(distinct.size > 1, `observed distinct model fingerprints=${distinct.size}`, "Compare a pinned model/version target against the alias with otherwise identical configuration.");
        }
    }
}
function observed(signatureObserved, signature, suggestedExperiment) {
    return { observable: true, signatureObserved, signature, suggestedExperiment };
}
function unobservable(signature, suggestedExperiment) {
