...
import { buildRuhrohClaimIndexEntryV2, buildRuhrohClaimIndexV2, buildRuhrohPublicationBundleV2, buildRuhrohPublicationV2, buildRuhrohPublishCheckV2, ruhrohPublishCheckRemediationCatalog, ruhrohPublishCheckRemediationForBlocker, validateRuhrohPublishBundle, verifyRuhrohBenchmarkClaimSources, } from "./publication.js";
import { loadRuhrohRerunLedger } from "./rerun-ledger.js";
import { aggregateRuhrohRuns, buildRuhrohCompareV2, normalizeRuhrohEvalResult, summarizeRuhrohBenchmarkClaimReadiness, summarizeRuhrohPairwiseAdapterComparisons, summarizeRuhrohReviewQueue, summarizeRuhrohRun, summarizeRuhrohSuiteAdapters, validateRuhrohBenchmarkClaim, validateRuhrohBenchmarkSummary, } from "./results.js";
import { buildRuhrohOutcomeFrontier } from "./economics.js";
...
import { validateRuhrohControlSurface, validateRuhrohWorkloadBinding, } from "./decision.js";
import { budgetCapabilityErrors, sha256CanonicalJson, validateAdapterManifest, } from "./economics-runtime.js";
import { RUHROH_ECONOMICS_COMMANDS, runRuhrohEconomicsCommand, } from "./economics-cli.js";
...
const BENCHMARK_TARGET_STREAMS = ["harness-controlled", "model-controlled", "native-stack", "recommended-stack", "custom"];
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (!arg.startsWith("-") && options.command === "economics") {
            if (options.economicsCommand === undefined) {
                if (!RUHROH_ECONOMICS_COMMANDS.includes(arg)) {
                    throw new Error(`Unknown economics command: ${arg}`);
                }
                options.economicsCommand = arg;
                continue;
            }
...
function runEconomicsCommand(options, deps) {
    ...
    try {
        ...
        if (!result.ok) {
            for (const error of result.errors)
                deps.stderr.write(`ruhroh economics ${options.economicsCommand}: ${error}\n`);
            return 1;
        }
        for (const warning of result.warnings)
            deps.stderr.write(`ruhroh economics ${options.economicsCommand} warning: ${warning}\n`);
        return 0;
    }
    catch (error) {
        deps.stderr.write(`ruhroh failed: ${error instanceof Error ? error.message : String(error)}\n`);
...
function runValidateTargetsCommand(options, deps) {
    if (options.inputPath === undefined) {
...
function claimIndexBenchmarkContext(claim) {
...
function uniqueSortedClaimContext(values) {
...
function claimIndexRenderView(report) {
...
function formatClaimIndexReportHtml(input, htmlPath) {
...
function formatClaimIndexBenchmarkContext(claim) {
    return [
...
function artifactBenchmarkTargetChecks(name, filePath, value) {
    if (value === undefined) {
        return [];
    }
    if (!isRecord(value)) {
...
async function runDemoCommand(options, deps) {
    try {
...
function resolveDemoModel(options, env) {
...
async function resolveDemoOpenRouterKey(deps, preflightBeforePrompt) {
...
function loadDemoDotEnvFiles(cwd) {
    const loaded = {};
...
function parseDemoDotEnvValue(rawValue) {
...
function readMaskedLine(deps, prompt) {
    const stdin = deps.stdin ?? process.stdin;
...
function demoAiderFiles(workspacePath) {
    return [
        "package.json",
        "index.html",
        "src/bookmarks.js",
...
function publishDemoWorkspace(liveWorkspacePath, workspacePath) {
    rmSync(workspacePath, { recursive: true, force: true });
    mkdirSync(workspacePath, { recursive: true });
    copyDirectoryContents(liveWorkspacePath, workspacePath);
}
function appendPathList(existing, itemPath) {
    if (existing === undefined || existing.trim().length === 0) {
        return itemPath;
    }
    return `${existing}${path.delimiter}${itemPath}`;
}
function preflightDemoLocalTools(deps) {
...
function resolveDemoPythonBin(deps) {
    const candidates = uniquePreserveOrder([
        deps.env.RUHROH_DEMO_PYTHON,
        ...DEMO_PYTHON_CANDIDATES,
    ].filter((candidate) => candidate !== undefined && candidate.trim().length > 0));
    const failures = [];
...
function formatWorkflowDocLabel(doc) {
...
function publicationEconomicTraceReferences(claim, paths) {
    ...
    return resultArtifacts.flatMap((rawArtifact) => {
        if (!isRecord(rawArtifact) || !Array.isArray(rawArtifact.artifactInventory))
            return [];
        return rawArtifact.artifactInventory.flatMap((rawItem) => {
            if (!isRecord(rawItem) || rawItem.name !== "economicTrace" || rawItem.available !== true)
                return [];
            ...
            if (!existsSync(absolutePath) || !statSync(absolutePath).isFile())
                return [];
            return [publicationArtifactReference("economic-trace", "ruhroh_economic_trace_span_v1", absolutePath, paths.publicationPath)];
        });
    });
}
function publicationArtifactReference(role, contractVersion, artifactPath, publicationPath) {
    return {
        role,
...
function formatPublishBundleCompareHtml(report, htmlPath) {
    const compare = report.compare;
    const suite = isRecord(compare.suite)
        ? { suite: compare.suite, source: {}, warnings: stringArrayField(compare, "suiteWarnings") }
        : undefined;
    const runPlan = isRecord(compare.runPlan) ? compare.runPlan : undefined;
    const claimReadiness = isRecord(compare.claimReadiness) ? compare.claimReadiness : undefined;
    const benchmarkClaim = isRecord(compare.benchmarkClaim) ? compare.benchmarkClaim : undefined;
    ...
    const resultArtifacts = source !== undefined && Array.isArray(source.resultArtifacts)
        ...
        : [];
    return formatCompareReportHtml(Array.isArray(compare.groups) ? compare.groups : [], suite, Array.isArray(compare.suiteAdapterSummaries) ? compare.suiteAdapterSummaries : [], Array.isArray(compare.pairwiseComparisons) ? compare.pairwiseComparisons : [], Array.isArray(compare.reviewQueue) ? compare.reviewQueue : [], claimReadiness, runPlan, stringArrayField(compare, "runPlanWarnings"), resultArtifacts, htmlPath, isRecord(compare.outcomeFrontier) ? compare.outcomeFrontier : undefined);
}
function publishBundleArtifactPathMap(originalClaim, localizedClaim) {
...
function publishBundleClaimResultArtifacts(claim) {
    const source = isRecord(claim.source) ? claim.source : undefined;
...
function localizeReviewQueueReportForBundle(report, pathMap) {
    return {
        ...report,
...
function localizeEvalQualityReportForBundle(report, pathMap) {
    return {
        ...report,
        source: { ...report.source, resultsPath: "sources/results" },
...
function localizeCalibrationCaseFile(item, field, paths, caseDir, role, sourceFiles) {
    ...
    copyPublishBundleSourceFile(originalPath, paths, relativeTargetPath, role, `Reviewer calibration ${field === "inputPath" ? "input" : "output"} for ${stringField(item, "caseId") ?? "case"}.`, sourceFiles);
...
function buildPublishBundleManifest(report, paths, sourceFiles = []) {
    const files = [
        {
            role: "manifest",
...
function formatPublishBundleReadme(manifest, report, cwd) {
    ...
    lines.push("", "## Review Order", "", "- Open ruhroh-compare.html for the aggregate outcome, intervals, blockers, and evidence browser.", "- Open ruhroh-eval-quality.html to inspect reviewer evidence warnings before citing the claim.", "- Open ruhroh-review.html for the human review queue and required review items.", "- Inspect sources/ for the hashed run plan, reviewer check report, result JSON, transcripts, review outputs, and saved evidence inventory.");
    lines.push("", "## Files", "");
...
function loadWorkloadBinding(filePath) {
    const raw = readJsonObject(filePath);
    const binding = raw;
...
function loadAdapterManifest(filePath) {
    const raw = readJsonObject(filePath);
    const manifest = raw;
...
function runPlanSampleBenchmarkTargetWarnings(sample) {
...
function runPlanBenchmarkTargetWarnings(sampleId, plannedTarget, summary) {
...
function compareTargetModel(warnings, sampleId, prefix, planned, actual) {
...
function compareActualModel(warnings, sampleId, requested, actual) {
    for (const field of ["provider", "model", "canonicalId", "protocol", "version"]) {
...
function compareTargetProviderPath(warnings, sampleId, planned, actual) {
...
function compareTargetRecommendedStack(warnings, sampleId, planned, actual) {
...
function compareTargetString(warnings, sampleId, field, planned, actual) {
...
function providerPathDescriptor(value) {
...
function resolveAdapterSelections(options, cwd, env) {
    ...
    return adapters.map((adapter) => attachAdapterManifest(resolveAdapterSelection(adapter, cwd, env), options.adapterManifestPath, cwd));
}
function resolveBenchmarkTargetSelections(options, cwd, env) {
    if (options.targetConfigPath === undefined) {
        return [];
    }
    ...
    return targets
        ...
        .map((target) => attachAdapterManifest(resolveBenchmarkTargetSelection(target, cwd, env), target.adapterManifestPath ?? options.adapterManifestPath, cwd));
}
function attachAdapterManifest(selection, explicitPath, cwd) {
...
function resolveBenchmarkTargetSelection(target, cwd, env) {
...
function benchmarkTargetSelectionErrors(configStream, targets, options) {
    const targetStreams = uniquePreserveOrder(targets.flatMap((target) => target.stream === undefined ? [] : [target.stream]))
...
function loadBenchmarkTargetConfig(configPath) {
...
function benchmarkTargetConfigSourceErrors(raw) {
    const errors = [];
...
function validateBenchmarkTargetConfigFile(configPath) {
    const raw = readJsonObject(configPath);
    const errors = [];
    const warnings = [];
    const summaries = [];
...
function readBenchmarkTargetStream(value, errors) {
    if (value === undefined) {
        return undefined;
    }
...
function isBenchmarkTargetStream(value) {
    return typeof value === "string" && BENCHMARK_TARGET_STREAMS.includes(value);
}
function benchmarkTargetStreamErrors(stream, targets) {
    if (stream === "custom") {
        return [];
    }
    const errors = [];
...
function benchmarkHarnessControlIdentity(target) {
    const name = target.harness?.name ?? "unknown";
    const version = target.harness?.version ?? "unknown";
    return `${name}@${version}`;
}
function benchmarkModelControlIdentity(model) {
    if (model === undefined) {
        return "unknown";
    }
...
function benchmarkProviderPathControlIdentity(providerPath) {
...
function benchmarkRecommendedStackFlag(value) {
    if (typeof value === "boolean" || value === undefined) {
        return value;
    }
    return value.recommended;
}
function summarizeBenchmarkTargetForValidation(target) {
...
function benchmarkTargetMetadataWarnings(target, label) {
    return [
...
function normalizeBenchmarkTarget(value, label) {
    if (!isRecord(value)) {
...
function normalizeControlSurface(value, label) {
    if (value === undefined) {
        return undefined;
    }
    if (!isRecord(value)) {
...
function normalizeOptionalBenchmarkTargetStream(value, label) {
    if (value === undefined) {
        return undefined;
    }
    if (isBenchmarkTargetStream(value)) {
...
function normalizeBenchmarkHarness(value, label) {
    if (value === undefined) {
        return undefined;
    }
...
function normalizeBenchmarkModel(value, label, optional) {
    if (value === undefined) {
        if (optional) {
            return undefined;
        }
...
function normalizeProviderPath(value, label) {
...
function normalizeRecommendedStack(value, label) {
...
function readRequiredIdentifier(value, key, label) {
    const raw = value[key];
...
function maintainedAdapterCommand(adapterId) {
...
function formatReviewQueueReport(report, cwd) {
    ...
    lines.push("", "Next steps:", "  inspect the listed transcripts, event logs, eval output, and final workspace artifacts", "  record the human review decision and rationale with the benchmark pack or claim review", "  rerun ruhroh publish-check before publishing the claim");
    return `${lines.join("\n")}\n`;
}
...
function formatCompareReport(groups, compareSuite, suiteAdapterSummaries = [], pairwiseComparisons = [], reviewQueue = [], claimReadiness, runPlan, runPlanWarnings = [], outcomeFrontier) {
    ...
    if (outcomeFrontier !== undefined) {
        lines.push("", "Outcome frontier:", `  status: ${outcomeFrontier.status}`, `  reasons: ${formatList(outcomeFrontier.reasonCodes)}`, `  pareto: ${formatList(outcomeFrontier.paretoFrontierTargetIds)}`, `  robustPareto: ${formatList(outcomeFrontier.robustFrontierTargetIds)}`);
...
function formatCompareReportHtml(groups, compareSuite, suiteAdapterSummaries = [], pairwiseComparisons = [], reviewQueue = [], claimReadiness, runPlan, runPlanWarnings = [], resultArtifacts = [], htmlPath, outcomeFrontier) {
    return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
...
function outcomeFrontierHtml(frontier) {
...
function formatBenchmarkTargetSummary(target) {
    if (target === undefined) {
        return "unknown";
    }
...
function benchmarkTargetRecommended(target) {
...
function formatBenchmarkTargetHarness(harness) {
...
function formatBenchmarkTargetModel(label, model) {
    if (model === undefined) {
        return undefined;
    }
    ...
    const identity = canonicalId
        ?? ([provider, name].filter((part) => part !== undefined).join("/") || version);
...
function formatPairwiseVariables(comparison) {
