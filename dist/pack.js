...
import { discoverRuhrohScenarios, validateRuhrohScenarioSource, } from "./generate.js";
import { discoverRuhrohSuites, validateRuhrohSuiteSource, } from "./suites.js";
export function inspectRuhrohBenchmarkPack(input) {
...
function riskReviewBlockers(scenarios, suites) {
    return [
        ...scenarios.flatMap((scenario) => scenario.riskReview.warnings.map((warning) => (`${scenario.id ?? scenario.sourcePath}: risk review requirement failed: ${warning}`))),
        ...suites.flatMap((suite) => suite.riskReview.warnings.map((warning) => (`${suite.id ?? suite.sourcePath}: risk review requirement failed: ${warning}`))),
    ];
}
function calibrationBlockersForScenario(scenario) {
...
function formatScenarioInspection(result) {
...
function scenarioRiskReview(scenario) {
...
function suiteRiskReview(suite) {
...
function riskReviewWarnings(field, value, messages) {
...
function inspectScenarioContent(result) {
...
function resolvePromptPath(result, rawScenario) {
...
function fingerprintDeclaredContentPath(scenarioDir, declaredPath) {
...
function fingerprintDirectory(sourcePath, declaredPath) {
...
function readRawScenarioJson(scenarioPath) {
    try {
...
function formatSuiteInspection(result, scenarioDifficultyById, scenarioRuntimeById) {
...
function difficultyCounts(values) {
    const counts = {
        intro: 0,
        standard: 0,
        hard: 0,
...
function runtimeEstimate(values) {
