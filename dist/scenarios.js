...
export function lintRuhrohScenarioEvaluationDetailed(scenario) {
    ...
    const rawEvaluation = scenario.evaluation;
    ...
    if (scenarioContext.length < 2) {
        addDiagnostic("evaluation_context_minimum", "calibration", "evaluation.scenarioContext", "evaluation.scenarioContext should include at least 2 context notes for evaluator calibration");
    }
    if (calibrationCases.length === 0) {
        addDiagnostic("evaluation_calibration_cases_minimum", "calibration", "evaluation.calibrationCases", "evaluation.calibrationCases should include at least 1 expected judgment anchor");
    }
    if (goalRubric.length < 3) {
        addDiagnostic("evaluation_goal_rubric_minimum", "rubric", "evaluation.goalRubric", "evaluation.goalRubric should include at least 3 concrete outcome criteria");
    }
    if (evidenceGuidance.length < 2) {
        addDiagnostic("evaluation_evidence_guidance_minimum", "evidence", "evaluation.evidenceGuidance", "evaluation.evidenceGuidance should include at least 2 evidence collection instructions");
    }
    goalRubric.forEach((criterion, index) => {
        const normalized = criterion.trim().toLowerCase();
        if (normalized.length < 40) {
            addDiagnostic("evaluation_goal_rubric_terse", "rubric", `evaluation.goalRubric[${index}]`, `evaluation.goalRubric[${index}] is terse; prefer a concrete, auditable outcome`);
        }
        if (/\bsatisf(?:y|ies)\s+the\s+user\s+(?:goal|prompt|request)\b/u.test(normalized)) {
            addDiagnostic("evaluation_goal_rubric_generic", "rubric", `evaluation.goalRubric[${index}]`, `evaluation.goalRubric[${index}] is generic; state the specific behavior the evaluator must verify`);
        }
    });
    ...
    if (behaviorProxyGuidance) {
        addDiagnostic("evaluation_evidence_proxy_source", "evidence", "evaluation.evidenceGuidance", "evaluation.evidenceGuidance should prioritize delivered behavior over source-text or filename checks");
    }
    return dedupeEvaluationLintDiagnostics(diagnostics);
}
function validateCalibrationCases(value) {
...
function dedupeEvaluationLintDiagnostics(diagnostics) {
    const seen = new Set();
...
function validateScenarioMetadata(metadata) {
    const errors = [];
    ...
    if (record.expectedRuntimeSeconds !== undefined
...
function validatePublishedScenarioMetadata(scenario) {
    ...
    if (metadata.visibility === "held_out"
...
function validateScenarioLifecycle(value) {
    if (!isRecord(value)) {
...
function readCalibrationCases(value) {
    if (!Array.isArray(value)) {
        return [];
    }
