...
export function canonicalizeRuhrohDecimal(value) {
    return formatDecimal(parseDecimal(value));
}
export function addRuhrohDecimals(left, right) {
    return formatDecimal(addDecimal(parseDecimal(left), parseDecimal(right)));
}
export function subtractRuhrohDecimals(left, right) {
...
export function compareRuhrohDecimals(left, right) {
    const difference = parseDecimal(subtractRuhrohDecimals(left, right)).coefficient;
    return difference < 0n ? -1 : difference > 0n ? 1 : 0;
}
export function normalizeRuhrohBillingRecordsV2(records, profile) {
    ...
    for (const [index, record] of [...records].entries()) {
        ...
        if (typeof rawAmount !== "string")
            errors.push(`records[${index}] amount must be a decimal string`);
        else {
            try {
                amountDecimal = canonicalizeRuhrohDecimal(rawAmount);
            }
            catch (error) {
                errors.push(`records[${index}] has invalid amount: ${message(error)}`);
            }
        }
        ...
        if (sourceRowId === undefined || amountDecimal === undefined || currency === undefined || kind === undefined)
            continue;
...
export function buildRuhrohCostReconciliationV2(input) {
    ...
    if (input.billingSource.rowCount !== input.billingRows.length)
        blockers.push("billing source rowCount does not match normalized rows");
    const observedCurrencies = unique(input.billingRows.map((row) => row.currency)).sort();
    if (JSON.stringify([...input.billingSource.currencies].sort()) !== JSON.stringify(observedCurrencies))
        blockers.push("declared currencies do not match normalized rows");
    ...
    for (const join of joins)
        rowClasses.set(join.sourceRowId, join.matchClass);
    const count = (matchClass) => [...rowClasses.values()].filter((value) => value === matchClass).length;
    if (count("ambiguous") + count("unmatched") > 0)
        blockers.push(`${count("ambiguous") + count("unmatched")} billing row(s) remain ambiguous or unmatched`);
    const coverage = {
...
export function validateRuhrohBillingSourceManifestV2(value) {
    const errors = [];
    if (value.version !== "ruhroh_billing_source_manifest_v2")
        errors.push("version must be ruhroh_billing_source_manifest_v2");
    if (!nonEmpty(value.sourceId) || !nonEmpty(value.externalSchemaVersion))
        errors.push("sourceId and externalSchemaVersion are required");
    if (!["csv", "parquet", "records", "normalized_rows"].includes(value.format))
        errors.push("format is unsupported");
    if (!(Date.parse(value.billingPeriod.startedAt) < Date.parse(value.billingPeriod.endedAt)))
    ...
    if (new Set(value.currencies).size !== value.currencies.length || value.currencies.some((currency) => !/^[A-Z]{3}$/u.test(currency)))
        errors.push("currencies must be unique uppercase three-letter codes");
    if (!["internal", "restricted"].includes(value.privacyClassification))
        errors.push("raw billing sources cannot be public");
    errors.push(...validateRef(value.sourceRef, "sourceRef"));
    return unique(errors);
}
export function validateRuhrohBillingMappingProfileV2(value) {
    const errors = [];
    if (value.version !== "ruhroh_billing_mapping_profile_v2")
        errors.push("version must be ruhroh_billing_mapping_profile_v2");
    if (!nonEmpty(value.profileId) || !nonEmpty(value.provider) || !nonEmpty(value.externalSchemaVersion))
        errors.push("profile identity is incomplete");
    for (const field of ["sourceRowId", "amountDecimal", "currency", "kind"])
        if (!nonEmpty(value.fields[field]))
            errors.push(`fields.${field} is required`);
    if (!Number.isFinite(value.matching.boundedWindowSeconds) || value.matching.boundedWindowSeconds < 0)
        errors.push("boundedWindowSeconds must be non-negative");
    const allocationIds = new Set();
    for (const allocation of value.allocations) {
        if (!nonEmpty(allocation.sourceRowId) || allocationIds.has(allocation.sourceRowId))
            errors.push(`allocation ${allocation.sourceRowId} is invalid or duplicated`);
        allocationIds.add(allocation.sourceRowId);
        if (allocation.targets.length === 0 || new Set(allocation.targets.map((target) => target.workloadId)).size !== allocation.targets.length)
            errors.push(`allocation ${allocation.sourceRowId} requires unique targets`);
        for (const target of allocation.targets) {
            if (!nonEmpty(target.workloadId))
                errors.push(`allocation ${allocation.sourceRowId} target workloadId is required`);
            try {
                canonicalizeRuhrohDecimal(target.assignedAmountDecimal);
            }
            catch (error) {
                errors.push(`allocation ${allocation.sourceRowId} has invalid assigned amount: ${message(error)}`);
            }
            if (target.weightDecimal !== undefined) {
                try {
                    canonicalizeRuhrohDecimal(target.weightDecimal);
                }
                catch (error) {
                    errors.push(`allocation ${allocation.sourceRowId} has invalid explanatory weight: ${message(error)}`);
                }
            }
        }
    }
    const owners = new Map();
    for (const kind of FACT_KINDS_V2)
        for (const raw of value.kindValues[kind] ?? []) {
            const prior = owners.get(raw);
            if (!nonEmpty(raw) || (prior !== undefined && prior !== kind))
                errors.push(`kind value ${raw} is invalid or mapped more than once`);
            owners.set(raw, kind);
        }
    return unique(errors);
}
export function validateRuhrohNormalizedBillingRowV2(value) {
    const errors = [];
    if (value.version !== "ruhroh_normalized_billing_row_v2")
        errors.push("version must be ruhroh_normalized_billing_row_v2");
    if (!nonEmpty(value.sourceRowId) || !sha256(value.sourceRowSha256))
        errors.push("source row identity is invalid");
    try {
        if (canonicalizeRuhrohDecimal(value.amountDecimal) !== value.amountDecimal)
            errors.push("amountDecimal must be canonical");
    }
    catch (error) {
        errors.push(`amountDecimal is invalid: ${message(error)}`);
    }
    if (!/^[A-Z]{3}$/u.test(value.currency))
        errors.push("currency must be an uppercase three-letter code");
    if (!FACT_KINDS_V2.includes(value.kind))
...
export function validateRuhrohCostReconciliationV2(value) {
    const errors = [];
    if (value.version !== "ruhroh_cost_reconciliation_v2")
        errors.push("version must be ruhroh_cost_reconciliation_v2");
    if (!nonEmpty(value.reconciliationId) || !Number.isFinite(Date.parse(value.createdAt)))
        errors.push("reconciliation identity is invalid");
    ...
    for (const summary of value.currencies) {
        try {
            if (subtractRuhrohDecimals(summary.assignedTotalDecimal, summary.sourceTotalDecimal) !== summary.differenceDecimal)
                errors.push(`${summary.currency} difference is inconsistent`);
            if (summary.differenceDecimal !== "0")
                errors.push(`${summary.currency} assigned total does not equal source total`);
        }
        catch (error) {
            errors.push(`${summary.currency} totals are invalid: ${message(error)}`);
        }
    }
    ...
    if (value.ready !== shouldBeReady)
        errors.push(`ready must be ${shouldBeReady}`);
    return unique(errors);
}
function reconcileRow(row, facts, allocation, profile, blockers) {
    if (allocation !== undefined) {
        const assigned = sumDecimals(allocation.targets.map((target) => target.assignedAmountDecimal));
        if (assigned !== row.amountDecimal)
            blockers.push(`allocation ${row.sourceRowId} assigned amount ${assigned} does not equal source ${row.amountDecimal}`);
    ...
    if (exact.length > 1)
        return [joined(row, "ambiguous", exact, "provider request identifier matched multiple facts")];
    ...
    if (bounded.length > 1)
        return [joined(row, "ambiguous", bounded, "bounded match returned multiple facts")];
...
function currencySummary(currency, rows, joins, blockers) {
    ...
    if (differenceDecimal !== "0")
        blockers.push(`${currency} reconciliation differs from source by ${differenceDecimal}`);
    return {
        currency, sourceTotalDecimal, assignedTotalDecimal, differenceDecimal,
        byMatchClass: Object.fromEntries(MATCH_CLASSES.map((matchClass) => [matchClass, sumDecimals(currencyJoins.filter((join) => join.matchClass === matchClass).map((join) => join.assignedAmountDecimal))])),
        byFactKind: Object.fromEntries(FACT_KINDS_V2.map((kind) => [kind, sumDecimals(sourceRows.filter((row) => row.kind === kind).map((row) => row.amountDecimal))])),
    };
}
function parseDecimal(value) {
    const match = /^([+-]?)(\d+)(?:\.(\d*))?(?:[eE]([+-]?\d+))?$/u.exec(value.trim());
    if (match === null)
        throw new Error("expected a finite base-10 decimal string");
...
function sumDecimals(values) { return values.reduce(addRuhrohDecimals, "0"); }
function resolveKind(raw, values) { return FACT_KINDS_V2.find((kind) => (values[kind] ?? []).includes(raw)); }
function optionalProperties(optional) {
    const result = {};
    for (const field of ["occurredAt", "providerRequestId", "principalRef", "workloadId", "model", "sku"]) {
...
const FACT_KINDS_V2 = ["charge", "credit", "refund", "commitment", "tax", "prepaid", "capacity", "adjustment"];
...
function stringValue(value) { const result = typeof value === "string" ? value.trim() : ""; return result.length === 0 ? undefined : result; }
function validateRef(value, label) { return [...(!nonEmpty(value?.path) ? [`${label}.path is required`] : []), ...(!sha256(value?.sha256) ? [`${label}.sha256 must be lowercase SHA-256`] : [])]; }
...
//# sourceMappingURL=billing-v2.js.map