...
export function runRuhrohFocusValidation(input) {
    ...
    if (input.runner === undefined)
        return unavailable(["FOCUS validator runner is not configured"]);
    if (!existsSync(input.dataFilePath))
        return unavailable(["FOCUS data file does not exist"]);
    if (!existsSync(input.modelFilePath))
        return unavailable(["pinned FOCUS model file does not exist"]);
    if (!existsSync(input.ruleSetPath))
        return unavailable(["packaged FOCUS rule-set directory does not exist"]);
    const modelDigest = createHash("sha256").update(readFileSync(input.modelFilePath)).digest("hex");
    if (modelDigest !== input.specLock.specification.model.sha256 || modelDigest !== input.conformanceProfile.modelSha256)
        return unavailable(["pinned FOCUS model hash does not match lock and conformance profile"]);
    const inputDigest = createHash("sha256").update(readFileSync(input.dataFilePath)).digest("hex");
    if (inputDigest !== input.inputRef.sha256)
        return unavailable(["FOCUS data file hash does not match inputRef"]);
    if (input.runner.timeoutMs < 1 || input.runner.timeoutMs > 3_600_000)
        return unavailable(["validator timeout must be between 1 ms and one hour"]);
    ...
    if (probe.error !== undefined || probe.status !== 0)
        return unavailable([`validator version probe failed: ${sanitize(probe.error?.message ?? probe.stderr ?? "unknown error")}`]);
    if (probe.stdout.trim() !== input.runner.versionProbe.expectedOutput || probe.stdout.trim() !== input.specLock.validator.version)
        return unavailable([`validator version probe returned ${sanitize(probe.stdout.trim()) || "empty output"}; expected ${input.specLock.validator.version}`]);
    ...
    try {
        ...
        if (result.error !== undefined || !existsSync(outputPath))
            return unavailable([`validator execution failed: ${sanitize(result.error?.message ?? result.stderr ?? `exit ${result.status}`)}`]);
        let parsed;
        try {
            parsed = parseRuhrohFocusJUnit(readFileSync(outputPath, "utf8"));
        }
        catch (error) {
            return unavailable([`validator output is malformed: ${sanitize(message(error))}`]);
        }
...
export function parseRuhrohFocusJUnit(xml) {
    ...
    for (const item of cases) {
        ...
        if (!/^[A-Za-z0-9_.:-]+$/u.test(ruleId))
            throw new Error("testcase has an invalid rule ID");
        const body = item[2] ?? "";
        const child = /<(failure|skipped|error)\b([^>]*)/u.exec(body);
        const status = child?.[1] === "failure" ? "failed" : child?.[1] === "error" ? "error" : child?.[1] === "skipped" ? "skipped" : "passed";
