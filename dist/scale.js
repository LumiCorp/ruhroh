export const RUHROH_REFERENCE_SCALE_LEVELS = [1, 2, 4, 8, 16];
export function validateRuhrohScaleExperiment(experiment) {
    const errors = [];
...
function analyzeTarget(experiment, targetId, observations) {
...
function distributionSummary(values, suffix) {
...
function percentileSummary(values, suffix) {
    return values.length === 0 ? {} : {
        [`p50${suffix}`]: percentile(values, 0.5),
        [`p95${suffix}`]: percentile(values, 0.95),
    };
}
function fitCandidates(levels) {
    const candidates = ["T(1)", "T(log_n)", "T(n)", "T(n_k)", "T(n_k_a)"];
...
function bootstrapBestCandidates(experiment, targetId, observations) {
    ...
    for (let iteration = 0; iteration < experiment.bootstrapSamples; iteration += 1) {
        const levels = experiment.levels.map((level) => {
            ...
            const resampled = Array.from({ length: samples.length }, () => samples[Math.floor(random() * samples.length)]).filter((item) => item !== undefined);
...
function candidateBasis(candidate, level) {
    switch (candidate) {
        case "T(1)": return 1;
...
function fitCoefficient(points) {
...
function compareFits(left, right) {
    const complexityOrder = ["T(1)", "T(log_n)", "T(n)", "T(n_k)", "T(n_k_a)"];
...
function medianOptional(values, key) {
    return values.length > 0 && values.every((value) => value !== undefined)
        ? { [key]: percentile(values, 0.5) }
        : {};
}
